...
### DC solution
##t_step = 0.2e-6
##Adc = np.array([[0, -1 / L], [1 / C, -1 /(R*C)]])
##Bdc = np.array([[v_in/L], [0]])
##x_dc_i = [0, 0]
##
##delta = lambda t : 0.5
##def f_x_dc(t, x):
...
##                  [1/C, 0, -1/(R*C), w*k],
##                  [0, 1/C, -w*k, -1/(R*C)]])
##B_1st = np.array([[v_in/(2*np.pi*k*L)], [v_in/(2*np.pi*k*L)], [0], [0]])
##x_1st_i = [0, 0, 0, 0]
##
##delta_1st = lambda t : np.array([[np.sin(2*np.pi*k*0.5)],[np.cos(2*np.pi*k*0.5)-1], [0], [0]])
##def f_x_1st(t, x):
